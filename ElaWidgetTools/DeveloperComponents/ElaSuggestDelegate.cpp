...
void ElaSuggestDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    path.addRoundedRect(optionRect, 2, 2);
    ...
    painter->drawText(option.rect.x() + 37, option.rect.y() + 25-7, suggest->getSuggestText());
    ...
    if (suggest->getElaIcon() != ElaIconType::None)
    {
        ...
        painter->drawText(option.rect.x() + 11, option.rect.y() + 26-7, QChar((unsigned short)suggest->getElaIcon()));
    }
    painter->restore();
}
